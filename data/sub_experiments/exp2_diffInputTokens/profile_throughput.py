...
def main(args: argparse.Namespace):
    ...
    if not args.no_monitor:
        ...
        """
        measurement_state = energy_monitor.measurement_states.pop(f'{task_name}')
    ...
    if generated_outputs is not None:
        ...
        list_first_token_time = [round(generated_outputs[i].metrics.first_token_time - generated_outputs[i].metrics.first_scheduled_time, 3)for i in range(len(generated_outputs))] 
        # Collect the end time of the batch
        list_elapsed_time = [round(generated_outputs[i].metrics.finished_time - start_time, 3) for i in range(len(generated_outputs))]

    if generated_outputs is not None:
        df = pd.DataFrame({
            "num_in_tokens": list_num_in_tokens,
            "num_out_tokens": list_num_out_tokens,
            "first_token_time": list_first_token_time,
            "elapsed_time": list_elapsed_time
        })
        df.to_csv(os.path.join(os.path.curdir, f'data/mns{engine_args.max_num_seqs}_n{args.n_samples}_{os.path.basename(engine_args.model)}.csv'), index=False)
